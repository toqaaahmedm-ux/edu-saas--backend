...
import { RequestExcuseDto } from './dto/request-excuse.dto';
...
// third controller, scoped directly by attendance record id — used for the
// excuse request flow, which isn't naturally lesson- or course-scoped from
// the frontend's point of view (the student already has the record id)
@Controller('attendance')
export class AttendanceExcuseController {
  constructor(private readonly attendanceService: AttendanceService) {}

  @UseGuards(RolesGuard)
  @Roles(Role.STUDENT)
  @Post(':id/excuse')
  @AuditAction('ATTENDANCE_EXCUSE_REQUESTED')
  requestExcuse(
    @Param('id') id: string,
    @GetUser() user: any,
    @Body() body: RequestExcuseDto,
  ) {
    return this.attendanceService.requestExcuse(id, user.id, user.tenantId, body.reason, body.fileUrl);
  }

  @UseGuards(RolesGuard)
  @Roles(Role.TEACHER, Role.ADMIN)
  @Patch(':id/excuse/review')
  @AuditAction('ATTENDANCE_EXCUSE_REVIEWED')
  reviewExcuse(
    @Param('id') id: string,
    @GetUser() user: any,
    @Body() body: ReviewExcuseDto,
  ) {
    return this.attendanceService.reviewExcuse(id, user.tenantId, user.id, user.role, body.decision);
  }
}
